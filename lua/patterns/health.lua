...
health.ns = vim.api.nvim_create_namespace("patterns/health");
health.buffer = nil;
health.window = nil;
...
health.notify = function (handler, opts)
	...
	elseif handler == "trace" then
		...
		vim.api.nvim_echo(vim.list_extend({ { " patterns.nvim: ", "DiagnosticInfo" } }, opts.message), true, {});
	end
	---_
end
...
health.check = function ()
	...
	local symbols = require("patterns.symbols");
